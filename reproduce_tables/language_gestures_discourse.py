...
for u, la in tqdm(zip(all_texts_subsampled_gi, all_languages_subsampled_gi)):
    ...
    for w in u.split():
        if la == 'en':
            if w in disc_markers:
                single_cat.add(True)
            else:
                single_cat.add(False)
            
    discmark_y.append(any(single_cat))
...
#     y = y#[idx_for_subsample]
    ...
    n_obs_fold = list()
    p_values_fold = list()
    for train_fold_idcs, test_fold_idcs in stratified_group_k_fold(X=all_vid_ids_subsampled, y=y, groups=all_vid_ids_subsampled, k=10):    
...
pd.DataFrame(all_disc_markers).to_csv("discourse_markers_task_10x100.csv", index=False)