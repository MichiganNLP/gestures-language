...
geninc_path = "/home/public/DICTIONARIES/LingEthnography/GeneralInquirer/GeneralInquirer.all.txt"
dictionary = pd.read_csv(geninc_path, header=None, names=('word', 'category'))
# d_generalinc = dict(zip(dictionary['word'], dictionary['category']))
d_generalinc = pd.DataFrame(zip(dictionary['word'], dictionary['category'])).groupby(0)[1].unique().to_dict()

geninc_categories = list()

for u, la in tqdm(zip(all_texts_subsampled_gi, all_languages_subsampled_gi)):
    single_cat = set()
    for w in u.split():
        if la == 'en':
            c = d_generalinc.get(w)
        if c is not None:
            for sev_c in c:
                single_cat.add(sev_c)

    geninc_categories.append(single_cat)

all_performances_gi = list()

for hjk in range(10):
    perf = list()

    for cat in dictionary['category'].unique():
        y = [cat in c for c in geninc_categories]
...
#                     'wilcoxon_against_raw': wilcoxon(x=scores, y=raw_poses_scores, alternative = 'greater')[1],
    ...
    all_performances_gi.append(pd.DataFrame(perf))
    
pd.DataFrame(pd.concat(all_performances_gi)).to_csv("geninc_pred_task_v2.csv", index=False)